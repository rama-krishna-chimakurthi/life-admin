...
export default function AddTransactionModal({ onClose, editingTransaction }: any) {
  const { assets, addTransaction, updateTransaction, deleteTransaction, createAsset } = useStore();
  const [amount, setAmount] = useState(editingTransaction ? String(editingTransaction.amount) : "");
  const [category, setCategory] = useState<typeof categories[number]>(editingTransaction ? editingTransaction.category : "Expense");
  const [subcategory, setSubcategory] = useState(editingTransaction ? editingTransaction.subcategory || "" : "");
  const [showCustomSubcategory, setShowCustomSubcategory] = useState(false);
  const [customSubcategory, setCustomSubcategory] = useState("");
  const [assetId, setAssetId] = useState<string | null>(editingTransaction ? editingTransaction.assetId : ((assets || [])[0]?.id || null));
  const [fromAssetId, setFromAssetId] = useState<string | null>(editingTransaction ? editingTransaction.fromAssetId || null : ((assets || [])[0]?.id || null));
  const [toAssetId, setToAssetId] = useState<string | null>(editingTransaction ? editingTransaction.toAssetId || null : ((assets || [])[1]?.id || null));
  ...
  const onSave = () => {
    ...
    if (category === 'Transfer') {
      if (!fromAssetId || !toAssetId) {
        return Alert.alert("Select accounts", "Please select both from and to accounts for transfer.");
      }
      if (fromAssetId === toAssetId) {
    ...
    const transactionData = {
      ...
      category,
      subcategory,
      assetId: category === 'Transfer' ? fromAssetId! : 
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      >
        <Text style={{ fontSize: 18, fontWeight: "700" }}>{editingTransaction ? "Edit Transaction" : "Add Transaction"}</Text>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 6 }}>
        {subcategories[category].map((sub) => (
          ...
          >
            <Text style={{ color: subcategory === sub ? "#fff" : "#666" }}>{sub}</Text>
          </TouchableOpacity>
        ))}
      ...
      {category === 'Transfer' ? (
        <>
          ...
          <View style={{ borderWidth: 1, borderColor: "#e6eef7", borderRadius: 8, marginTop: 6 }}>
            {(assets || []).map((a) => (
              <TouchableOpacity
                key={a.id}
      ...
      <TouchableOpacity
        ...
        onPress={onSave}
      >
        <Text style={{ color: "#fff", fontWeight: "700" }}>{editingTransaction ? "Update Transaction" : "Save Transaction"}</Text>
      </TouchableOpacity>
