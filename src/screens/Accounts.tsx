    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#f6f8fb" }}>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <TouchableOpacity onPress={() => setShowForm(!showForm)}>
          <Text style={{ color: "#3778C2", fontSize: 24 }}>{showForm ? "×" : "+"}</Text>
        </TouchableOpacity>
      </View>

      {showForm && (
        <View style={{ backgroundColor: "#fff", padding: 16, borderRadius: 10, marginBottom: 16 }}>
          <Text style={{ fontWeight: "600", marginBottom: 8 }}>Add New Account</Text>
          ...
          <View style={{ flexDirection: "row", marginBottom: 12 }}>
            {["bank", "cash", "credit"].map((t) => (
              ...
              >
                <Text style={{ color: type === t ? "#fff" : "#666", textTransform: "capitalize" }}>{t}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          >
            <Text style={{ color: "#fff", fontWeight: "700" }}>Add Account</Text>
          </TouchableOpacity>
        </View>
      )}
      <FlatList
